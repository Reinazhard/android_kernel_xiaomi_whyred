...
static int smb2_parse_dt(struct smb2 *chip)
{
	...
	if (hwc_check_india == 1) {
		...
		if (of_find_property(node, "qcom,thermal-mitigation-china",
				     &byte_len)) {
			chg->thermal_mitigation =
				devm_kzalloc(chg->dev, byte_len, GFP_KERNEL);
			...
			chg->thermal_levels = byte_len / sizeof(u32);
			rc = of_property_read_u32_array(
				node, "qcom,thermal-mitigation-china",
				chg->thermal_mitigation, chg->thermal_levels);
			if (rc < 0) {
				dev_err(chg->dev,
					"Couldn't read threm limits rc = %d\n",
...
static void thermal_fb_notifier_resume_work(struct work_struct *work)
{
	...
	LctThermal = 1;
#if defined(CONFIG_XIAOMI_WHYRED)
	if ((lct_backlight_off) &&
	    (LctIsInCall == 0) /*&& (hwc_check_india == 1)*/) {
		if (hwc_check_india == 1) {
			if (lct_therm_lvl_reserved.intval >= 2)
				smblib_set_prop_system_temp_level(
					chg, &lct_therm_india_level);
			else
				smblib_set_prop_system_temp_level(
				     ...
				     unsigned long event, void *data)
{
	...
	struct smb_charger *chg =
		container_of(noti, struct smb_charger, notifier);
	int *blank;
...
static int smb2_remove(struct platform_device *pdev)
{
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		sysfs_remove_file(&chg->dev->kobj, &attrs[attr_count].attr);
	}
#endif
