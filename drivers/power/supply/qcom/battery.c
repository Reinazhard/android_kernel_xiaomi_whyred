...
static void handle_settled_icl_change(struct pl_data *chip)
{
	...
	int battery_temp;
	union power_supply_propval lct_pval = {
		0,
	};
#endif
	...
	if (chip->pl_mode == POWER_SUPPLY_PL_USBIN_USBIN) {
		rc = power_supply_get_property(
			chip->batt_psy, POWER_SUPPLY_PROP_TEMP, &lct_pval);
		if (rc < 0) {
		...
		pr_err("main_limited=%d, main_settled_ua=%d, chip->pl_settled_ua=%d ,total_current_ua=%d , battery_temp=%d\n",
		       main_limited, main_settled_ua, chip->pl_settled_ua,
		       total_current_ua, battery_temp);
		if ((main_limited &&
		     (main_settled_ua + chip->pl_settled_ua) < 1300000) ||
		    (main_settled_ua == 0) ||
		    ((total_current_ua >= 0) &&
			...
			if ((battery_temp > 20) && (battery_temp < 440)) {
				vote(chip->pl_enable_votable_indirect,
				     USBIN_I_VOTER, true, 0);
				vote(chip->pl_disable_votable, PL_TEMP_VOTER,
				     false, 0);
			} else {
				vote(chip->pl_enable_votable_indirect,
				     USBIN_I_VOTER, false, 0);
				vote(chip->pl_disable_votable, PL_TEMP_VOTER,
				     true, 0);
			}
		}
	...
	if ((main_limited &&
	     (main_settled_ua + chip->pl_settled_ua) < 1300000) ||
	    (main_settled_ua == 0) ||
	    ((total_current_ua >= 0) && (total_current_ua <= 1300000)))
		vote(chip->pl_enable_votable_indirect, USBIN_I_VOTER, false, 0);
	else
